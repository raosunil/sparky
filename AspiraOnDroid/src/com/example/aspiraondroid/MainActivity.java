...
public class MainActivity extends ActionBarActivity implements PlaceholderFragment.Callback {
	...
	public void callAction(Intent intent){
		StringBuilder logMessage = new StringBuilder(" Found USB Device");
			...
			if (tmpdevice.getVendorId()==VENDOR_ID && tmpdevice.getProductId()==PRODUCT_ID
				...
				try{
					...
					for (int i = 0; i < usbInterface.getEndpointCount(); i++) {
						logMessage.replace(0, logMessage.length(), "USB Connected Successfully");
	...
	public void monitorServiceClicked(View v) {
		...
		// Is the toggle on?
		toggleBtn = (ToggleButton)v;
	    boolean on = toggleBtn.isChecked();
	    Intent i = new Intent(getApplicationContext(), MonitorService.class);
	   // Intent readingServiceIntent = new Intent(getApplicationContext(), SpirometerReadingService.class);
	    ...
	    if (on) {
	    	...
	    	stopService(i);
	    	//stopService(readingServiceIntent);
	    }
	...
	public void onCreateGetNextReadingTextView(View v) {
		...
		if (!calledOnce && Long.valueOf(prefs.getString("asthmaglobals.nextReadingTimeinMillis", "0")) > 0){
	...
	protected void onResume(){
		...
		nextReading.setText(determineNextReading());
		if (toggleBtn!=null){
			boolean status_toggle = Boolean.valueOf(prefs.getString("last_toggle_status", "false"));
			toggleBtn.setChecked(status_toggle);
		}
	}
	...
	protected void onPause(){
		...
		if (toggleBtn!=null){
			SharedPreferences.Editor editor = prefs.edit();
			//Toast.makeText(getApplicationContext(), String.valueOf(cal.getTimeInMillis()), Toast.LENGTH_LONG).show();
			editor.putString("last_toggle_status", toggleBtn.isChecked()?"true":"false");
			editor.commit();
		}
	}
